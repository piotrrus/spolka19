...
describe('ContractorsService', () => {
     ...
     //OK
     it('should be call ContractorsService getList with get method', () => {
          const url = `${environment.api}${CONTRACTORS_API_PATHS.LIST}`;
     ...
     // OK
     it('should be call ContractorsService getDetails with get method', () => {
          const id = 2;
          const url = `${environment.api}${CONTRACTORS_API_PATHS.DETAILS}${id}`;
     ...
     it('should call ContractorsService remove() with delete method', () => {
          const id = 2;
          const url = `${environment.api}${CONTRACTORS_API_PATHS.DELETE}${id}`;
     ...
     //OK
     it('should call ContractorsService create() with post method', () => {
          // const id = 2;
          const contractor = contractorData;
          const url = `${environment.api}${CONTRACTORS_API_PATHS.CREATE}`;
          service.create(contractor).subscribe();
          const req = httpTestingController.expectOne(url, 'create api');
          expect(req.request.method).toBe('POST');
          req.flush(contractorData);
     });
     it('should call ContractorsService update() with post method', () => {
          const id = 2;
          const contractor = contractorData;
          const url = `${environment.api}${CONTRACTORS_API_PATHS.UPDATE}${id}`;
          service.update(id, contractor).subscribe();
          const req = httpTestingController.expectOne(url, 'create api');
          expect(req.request.method).toBe('POST');
          req.flush(contractorData);
     });
